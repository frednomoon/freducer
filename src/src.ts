...
const defaultLocationFunction: LocationFunction = (
  state,
  action,
  internal
) => ({
 ...
 */
export const asyncMethod = (parts: [string, string, string]) =>
  function (options: MethodOptions | undefined = {}): reducerMap {
    const { locationFunction = defaultLocationFunction } = options
    return {
      [parts[0]]: (state, action) => {
        return locationFunction(state, action, {
          pending: true,
        })
      },
      [parts[1]]: (state, action) => {
        return locationFunction(state, action, {
          ...defaultInitialState(options),
          error: action.payload,
        })
      },
      [parts[2]]: (state, action) => {
        return locationFunction(state, action, {
          ...defaultInitialState(options),
          success: true,
          data: action.payload,
 ...
 */
export default (parts: [string, string, string]) =>
  function (
    type: string,
    ...
    return reducer(
      {
        [type]: asyncMethod(parts)({ initialState, locationFunction }),
